...
import { conversations, orders } from "@/lib/mock/mock-data";
...
export default function StaffConsolePage() {
  return <main id="main-content" className="premium-canvas min-h-screen px-5 py-5 sm:px-8 sm:py-7 lg:px-12 xl:px-16"><div className="mx-auto max-w-[1440px]">
    ...
    <Reveal className="mb-8 mt-10 flex flex-wrap items-end justify-between gap-6" y={14}><div className="max-w-3xl"><div className="flex items-center gap-2"><span className="size-2 rounded-full bg-red-600 shadow-[0_0_0_5px_rgb(220_38_38_/_0.1)]" aria-hidden="true" /><p className="text-xs font-black uppercase tracking-[0.2em] text-red-700">Trung tâm vận hành</p></div><h1 className="mt-4 text-balance text-3xl font-black tracking-[-0.035em] sm:text-5xl">Phục vụ tức thì, trọn vẹn dịch vụ</h1><p className="mt-3 text-sm leading-6 text-zinc-500 sm:text-base">Trợ lý AI tự động nhận đơn siêu tốc. Nhân viên hỗ trợ trực tiếp khi khách hàng yêu cầu.</p></div><div className="premium-card flex items-center gap-3 rounded-2xl px-4 py-3 text-xs font-bold text-emerald-800"><span className="relative flex size-3"><span className="absolute inline-flex size-full animate-ping rounded-full bg-emerald-400 opacity-60" /><span className="relative inline-flex size-3 rounded-full bg-emerald-500" /></span><div><p>Kết nối Messenger thời gian thực</p><p className="mt-0.5 font-medium text-zinc-400">Phản hồi trung bình ~1.2 giây</p></div></div></Reveal>
    <Reveal delay={0.06}><StaffConsole initialConversations={conversations} initialOrders={orders} /></Reveal>
    <footer className="py-8 text-center text-xs text-zinc-400">Trang làm việc nhân viên · Dữ liệu mô phỏng đã được ẩn thông tin nhạy cảm</footer>
  </div></main>;
}
