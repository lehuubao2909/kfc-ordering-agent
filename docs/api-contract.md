...
BROWSING → CART ⇄ CONFIRMING → COLLECTING_DELIVERY → SELECTING_PAYMENT ─┬→ PLACED (cod)
                                    │ (món hết tại                       └→ AWAITING_PAYMENT → PLACED (qr/card)
                                    ↩ cửa hàng → sửa giỏ)
PLACED → PREPARING → DELIVERING → DELIVERED
CANCELLED: được phép từ AWAITING_PAYMENT, PLACED. Từ PREPARING trở đi: KHÔNG hủy.
MỚI 11/7: thêm transition COLLECTING_DELIVERY → CONFIRMING (store resolve phát hiện món hết → khách sửa giỏ).
```

## Store resolution (MỚI 11/7 — trong set_delivery_info, KHÔNG thêm state)

`set_delivery_info(address, phone)` giờ trả thêm ngữ cảnh cửa hàng:
```
...
| set_delivery_info | lưu customers + session → **resolve cửa hàng** (store-service: quận → giờ mở → tồn món) → OK: SELECTING_PAYMENT; món hết: → CONFIRMING gợi ý thay |
